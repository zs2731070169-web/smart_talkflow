...
> ⚠️ **本文档已作废(2026-06)**,被 [《传统业务系统接入 Agent 落地计划 v2.0》](传统业务系统接入%20Agent%20落地计划%20v2.0.md) 取代。
> v2.0 放弃了本文的「YAML 引擎 + 独立 adapter 服务 / MCP 独立注册中心 / Celery 长程」方案,改为全程基于编码层(`BaseWorkflow` + 进程内 adapter)演进。本文仅作历史决策记录保留,**不再作为落地依据**。
